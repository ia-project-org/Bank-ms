pipeline {
    ...
    environment {
        REPO_URL = 'https://github.com/ia-project-org/Bank-ms.git'
             ...
             steps {
                 script {
                     ...
                     withCredentials([usernamePassword(credentialsId: 'github', passwordVariable: 'GITHUB_PASSWORD', usernameVariable: 'GITHUB_USERNAME')]) {
                         ...
                         sh """
                             git status
                             sed -i 's|image: minaouimh/ai:.*|image: minaouimh/ai:${imageTag}|g' deployment/bank-ms/bank-server-deployment.yaml
                             git config --global user.email "jenkins@example.com"
                             git config --global user.name "Jenkins"
                             git add deployment/bank-ms/bank-server-deployment.yaml
